...
Stellar Core, or Tican-core, separates data into "current" and "historical."

Current data is the subject of peer-to-peer messages--consensus is only concerned with the present,
not the past. Current data _resides_ in a local SQL database paired with each Tican-core
process. This database is consulted and updated "live" in an ACID fashion as Tican-core applies
each [transaction](transaction.md) set for which consensus was reached and forms each new [ledger](ledger.md).

Unlike many similar systems, Tican-core does _not_ need to consult history in order to apply a
...
Historical data resides in _history archives_. A Tican-core process should almost always have
access to a history archive. While it is possible to run a Tican-core process without any
...
normal operations, a Tican-core process should always be configured with one or more history
archives.
...
Many different facilities or services can be used as history archives; Tican-core only needs to be
supplied with a way to "get" and "put" files to and from the archive. For example, history archives
may be FTP or SFTP servers, filesystem directories shared between Tican-core processes, AWS S3,
Google Cloud Storage, Azure Blob storage or similar commodity object storage services.

History archives are defined in a very lightweight fashion, in Tican-core's configuration file, by
providing a pair of `get` and `put` command templates. Tican-core will run the provided command
...
Tican-core leans heavily on the XDR data format. This is an old, stable, standardized
...
When storing XDR files to history archives, Tican-core first applies gzip (RFC 1952) compression
to the files. The resulting `.xdr.gz` files can be concatenated, accessed in streaming fashion, or
decompressed to `.xdr` files and dumped as plain text by Tican-core.


## Checkpointing

During normal operation, a validating Tican-core server will save a "checkpoint" of its recent
...
When Tican-core finds that it is out of sync with its peers--either because it is joining
...
another when out of sync. If you run a Tican-core server without configuring history archives, it
will never synchronize with its peers (unless they all start at the same time).

The peer-to-peer protocol among Tican-core peers deals only with the current ledger's transactions
...
One of the factors motivating the Tican-core history design was to permit other programs and 3rd
parties transparent, easy, and unbiased access to the ledger and transaction history, without having
to "go through" the Tican-core program or protocol. Any program that can fetch data from a history
archive and deserialize XDR can read the complete history; there is no need to speak the
Tican-core peer-to-peer protocol or interact with any Tican-core peers.
...
sequentially. Anyone wishing to audit or reconstruct the activity of Tican-core by monitoring a
history archive can simply poll the archive and consume new files as they arrive.

All XDR encoding and decoding in Tican-core is done by code generated automatically from the
associated [XDR schemas](/src/xdr); any other compliant XDR code generator should produce a
deserializer that can read and write the same history. The XDR code generator used in Tican-core
...
few additional, less obvious motives are at work in the design of the history system in Tican-core.
A few reasons that the extra effort of configuring independent history archives is, in our judgment, worth its slight awkwardness:
  ...
  - This design reduces the risk of lost peers. Tican-core peers are comparatively ephemeral: new ones can
    ...
    been archived). While Tican-core is designed to run as a highly fault-tolerant replicated
...
In total, each checkpoint number `0xwwxxyyzz` consists of the following files:
  ...
  - One results file, named by ledger number as `results/ww/xx/yy/results-wwxxyyzz.xdr.gz`. The file
    ...
    actually running the `Tican-core` transaction-apply logic.
